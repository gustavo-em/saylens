package com.gustavoem.lesingo.speech
...
class SpeechRecognitionModule(
    ...
    override fun onError(error: Int) {
      ...
      val code = when {
        ...
        error == SpeechRecognizer.ERROR_INSUFFICIENT_PERMISSIONS ->
          ERROR_PERMISSION
        // Recognition is asked for offline first, so reaching the network at
        // all means this device has no pack for the language and the online
        // service was the fallback. Off the network, that fallback is gone,
        // and saying so beats a blank "try again" the learner cannot act on.
        error == SpeechRecognizer.ERROR_NETWORK ||
          error == SpeechRecognizer.ERROR_NETWORK_TIMEOUT -> ERROR_NETWORK
  ...
  companion object {
    const val NAME = "LesingoSpeechRecognition"
    ...
    private const val ERROR_NETWORK = "E_SPEECH_NETWORK"
