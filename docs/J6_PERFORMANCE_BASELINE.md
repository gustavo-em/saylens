...
30 FPS. These measurements use a debug build and are directional rather than a
laboratory benchmark; CPU is a single instantaneous sample and scene, device
temperature, and Android memory reclamation can vary between runs.
...
```sh
./scripts/benchmark-android.sh 30
```
