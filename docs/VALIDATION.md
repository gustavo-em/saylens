...
- Application ID: `com.gustavoem.lesingo`
- React Native: 0.87.0
- React: 19.2.3
- Node.js: 24.11.1
- npm: 11.6.2
...
started `com.gustavoem.lesingo/.MainActivity`. Android reported the process
...
![Lesingo Android baseline running on a physical Samsung device](assets/android-baseline.png)
...
service reported camera `0` open by `com.gustavoem.lesingo` and the UI showed
...
![Lesingo camera settings running on a physical Samsung device](assets/android-camera-settings.png)
...
The device panel advertises 60 and 120 Hz modes. Lesingo requests the highest
