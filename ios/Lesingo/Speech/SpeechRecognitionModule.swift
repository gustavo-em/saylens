...
final class SpeechRecognitionModule: NSObject {
  private enum ErrorCode {
    ...
    static let network = "E_SPEECH_NETWORK"
  }
  ...
  private let stateQueue = DispatchQueue(label: "com.gustavoem.lesingo.speech")
  ...
  private func handle(result: SFSpeechRecognitionResult?, error: Error?) {
    ...
    // On-device recognition is used wherever the device supports it, so a
    // failure to reach Apple's service means this device has no local model
    // for the language and there is no network to fall back on. Naming that
    // beats a blank failure the learner cannot act on.
    if isOffline(error) {
      settleRejecting(
        code: ErrorCode.network,
        message: "Speech recognition needs a connection on this device.",
        error: error
      )
      return
    }
  ...
  private func isOffline(_ error: Error) -> Bool {
    ...
    guard failure.domain == NSURLErrorDomain else {
      // The speech framework wraps its own transport failures, and the URL
      // error underneath is the one that says what went wrong.
